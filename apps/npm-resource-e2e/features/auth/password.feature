Feature: Auth
  ...
  Scenario Outline:
    ...
    Then login has been called with <username> <password> <basic_token>
    And <package_name> manifest has been called with token <token>
  ...
  Scenario Outline:
    ...
    When we execute the command
    Then login has been called with <username> <password> <basic_token>
    And <package_name> has been uploaded with token <token>
    Examples:
      | username | password | basic_token           | token | package_name | version |
      | user     | p@ssw0rd | dXNlcjpwQHNzdzByZA==  | t0ken | my_package   | 0.0.0   |
